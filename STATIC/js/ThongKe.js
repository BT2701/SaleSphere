...
function displayHeaderBoxThongKe(response) {
  if (Object.keys(response).length > 0) {
    ...
    document.getElementById("TongSoSanPham").innerHTML =
      response.total_products_sold;
    document.getElementById("TongSoDoanhThu").innerHTML = formatPrice(
      parseInt(response.total_revenue)
    );
    document.getElementById("DanhGiaTichCuc").innerHTML =
      response.total_good_reviews;
...
function GetThongTinHeaderByDateRange(fromDate, toDate) {
  $.ajax({
    ...
    dataType: "json",
    data: {
      action: "GetThongTinHeaderByDateRange",
      fromDate: fromDate,
      toDate: toDate,
    },
    success: function (response) {
...
function GetAllLoaiSanPhamByDateRange(fromDate, toDate) {
  $.ajax({
    ...
    dataType: "json",
    data: {
      action: "GetAllLoaiSanPhamByDateRange",
      fromDate: fromDate,
      toDate: toDate,
    },
    success: function (response) {
...
function displayTopSellingProducts(products) {
  ...
  $.each(products, function (index, product) {
    ...
    row.append(
      '<td><a href="chi-tiet-san-pham.php?product=' +
        encodeURIComponent(product.tenSanPham) +
